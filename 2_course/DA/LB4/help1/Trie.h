...
class TTrie {
	...
	void CreateLinks() {
		...
		while (!queue.empty()) {
			...
			parent = parent->Fail;
			while (parent != nullptr && parent->Childrens[current->Symbol] == nullptr) {
				parent = parent->Fail;
			}
			if (parent == nullptr) {
				current->Fail = Root;
				...
				current->Fail = parent->Childrens[current->Symbol];
			}
			if (current->Fail->End) {
...
public:
	...
	void Find(std::vector<std::pair<uint32_t, std::pair<int, int>>>& text, std::vector<std::pair<int, int>>& output) {
		...
		for (int i = 0; i < count; ++i) {
			while (current->Fail != nullptr && current->Childrens[text[i].first] == nullptr) {
				current = current->Fail;
			}
			if (current->Childrens[text[i].first] != nullptr) {
				current = current->Childrens[text[i].first];
			}
			if (current->End) {
