	...
	for(auto& elem:globals){
		if(!elem.second.test(DEFINED)){
			...
			if(elem.second.test(PROC)){
				ferror_message+= "Error[01-1] in line "+ elem.second.line +": Procedure '" + elem.first + "' used, but not defined\n";
		...
		if(!elem.second.test(USED) && !elem.second.test(BUILT)){
			if(elem.second.test(PROC)){
				ferror_message+= "Warning[01-3] in line "+ elem.second.line +": Procedure '" + elem.first + "' is defined but not used\n";
				//Процедура  'f' определена, но не использована
				//Procedure 'f' is defined but not used
			}
			if(elem.second.test(VAR)){
				ferror_message+= "Warning[01-4] in line "+ elem.second.line +": Variable '" + elem.first + "' is defined, but not used\n";
		...
		default:
			...
			if(ref.empty()){
				...
				if(!ref.test(BUILT)){
					...
					ferror_message+= "Error[19-4] in line "+ S2->line +": Using the name of the built-in constant '" + S2->name + " as the name of the procedure\n";
					//Использование имени встроенной константы 'test' в качестве имени процедуры
					//Using the name of the built-in constant 'test' as the name of the procedure
					return 1;
				}
      		}else ref.set(USED);
	}
...
int tSM::p49(){ //    HSET -> ( set! $id
	string name = S3->name;
	if(scope == 2) if(locals.count(name)) return 0;
	if(scope == 1) if(params.count(name)) return 0;
	tgName& ref = globals[name];
	if(ref.empty()){
		ref = tgName(VAR|USED, S1->line);
		return 0;
	}else if(ref.test(VAR) && !ref.test(BUILT)){
		ref.set(USED);
		return 0;
	}else if(ref.test(BUILT | PROC)){
		ferror_message+= "Error[49-1] in line "+ S3->line +": The built-in procedure '" + 
			name + "' cannot be used as a variable name\n";
		...
		ferror_message+= "Error[49-2] in line "+ S3->line +": The procedure '" + name + "'- declared in line " + 
		...
		return 1;
	}else if(ref.test(BUILT | VAR)){
		ferror_message+= "Error[49-3] in line "+ S3->line +": The built-in constant '" + 
			name + "' cannot be used as a variable name\n";
		//Встроеная константа 'name' не может быть использована в качестве имени переменной
		// The built-in constant 'name' cannot be used as a variable name
		return 1;
	}
	return 0;}
...
int tSM::p72(){ //   PDPAR -> PDPAR $id
	...
	if(check){
		if(!((ref.types & (type*step)) == (type*step))){
			ferror_message+= "Error[72-1] in line "+ ref.line +": Using an invalid procedure parameter - id, instead of idq\n";
			//Использование неверного параметра процедуры - id, вместо idq
			//Using an invalid procedure parameter - id, instead of id
			return 1;
		}
...
int tSM::p74(){ //  VARDCL -> ( define $id
	...
	if(ref.empty()){
		...
		ferror_message+= "Error[74-2] in line "+ S3->line +": The built-in constant '" + name 
			+ "' cannot be used as a variable name\n";
		//Встроеная константа 'pi' не может быть использована в качестве имени переменной
		//The built-in constant 'pi' cannot be used as a variable name
		return 1;
	}else if(ref.test(PROC)){
		ferror_message+= "Error[74-3] in line "+ S3->line +": The procedure '" + name + "'- declared in line " +
			ref.line + ", cannot be used as a variable name\n";
		...
		if(ref.test(DEFINED)){
			ferror_message+= "Error[74-4] in line "+ S3->line +": Redefining the '" +
...
int tSM::p79(){ //   PCPAR -> ( define ( $id
	...
	if(ref.empty()){
		...
		ferror_message+= "Error[79-2] in line "+ ref.line +": Built-in constant '" + name
			 +"cannot be used as a procedure name\n";
		//Встроенная константа 'pi' не может быть использована в качестве имени процедуры
		//Built-in constant 'pi' cannot be used as a procedure name
		return 1;
	} else if(ref.test(BUILT)){
		ferror_message+= "Error[79-3] in line "+ S4->line +": Cannot override the built-in '" + name + "' procedure\n";
		//Невозможно переопределить встроенную процедуру 'name'
		//Cannot override the built-in 'name'procedure
		return 1;
	} else if(ref.test(PROC)){
		if(ref.test(DEFINED)){
			ferror_message+= "Error[79-4] in line "+ S4->line +": Redefining the '" +
