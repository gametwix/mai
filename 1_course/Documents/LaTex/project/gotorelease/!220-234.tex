...
конечном итоге нас\\ интересует - \tcbox[nobeforeafter, tcbox raise base]{ max \{A (i) | \p{1} ≤ i ≤ n\} }.
    \item Задать рекуррентность.\\
Для \tcbox[nobeforeafter, tcbox raise base]{\p{1}<= i <= n, A(i)=\p{1}+ max\{A(j)|\p{1} ≤ j < i and input(j)< input(i)\}}.
